...
* 只用一行数据的叫单行函数（如四舍五入只需要这一行数据）

* 要多行数据的叫多行函数或聚合函数或分组函数（如max需要所有行的数据）。
...
* 角度2：子查询是否被执行多次
  ...
  * 相关查询：比如员工工资高于本部门的平均工资，部门有很多个，员工和部门是有相关的
  
  * 非相关子查询：比如员工工资高于本公司的平均工资，部门只有一个，员工和公司之间由于部门区分，所以是非相关的
...
## 1.8 11种查询关键字的编写和执行顺序

编写顺序：

select-distinct-函数-from-join-on-where-group by-having-order by-limit

执行顺序：

from-on-join-where-group by-函数-having-select-distinct-order by-limit
