...
- 整体执行顺序：
  ...
  在node 环境中，有 process.nextTick(() => {}) 和 setImmediate(() => {}) 两个 api
  
  1. 同步任务，注意 Promise 在 resolve、reject 之前是同步，async 在 await 之前是同步
  ...
  5. 进入时时间轮循执行宏任务，每执行一个宏任务，如果产生了新的微任务，就会先执行完所有的微任务，再执行第二个宏任务；宏任务之间也是有顺序的，相同时间的定时器按照代码先后顺序执行
  
  6. 当所有最早的想用定时的定时器执行完后，结束第一次事件轮循，执行 setImmediate
...
作用域链：作用域嵌套形成的链条，作用是查找变量，先从自己的作用域中找，找不到再从内到外查找，直到炸掉全局作用域，找不到会报错 xxx is not defined 。
...
- 使用严格模式后，所有指向 window 均会改为 undefined ，webpack 和 vite 默认都是严格模式
...
* 默认绑定规则
  ...
  * 函数独立调用，指向 window，调用不是 "直接" 通过对象就是独立调用，如立即执行函数，内嵌函数，对象的方法的内嵌函数，return 的闭包，形参子函数且在父函数内被调用都是独立调用函数
    ...
    // 不直接通过对象来调用
    obj1.func1()();
    const o = obj1.func2;  o();  // 这种情况叫做隐式丢失，作为形参也是一样
    ```

* 隐式绑定规则：
  
  * 谁调用就指向谁 a.fun() 指向 a，a.b.fun() 指向b。

* 显示绑定规则：
  js函数本身是一个对象，有一个调用的方法噶 call()，用来控制 this 指向：
  
  ```
  // 下面两种函数的调用方式是等价的，第一种是第二种的语法糖
  func(...);
  func.call(this指向, ...);
  ```
  
  和 call 类似的还有 apply 和 bind ：
  
  ```
  // 下面三种是等价的
  func.call(this指向, 形参….);
  func.apply(this指向, 形参数组);
  const f = func.bind(this指向, 形参….); f();
  ```

* new 绑定：
  构造函数的 this 指向 new 出来的对象。若构造函数内有 return 且不是 return this; 就会改变 this 指向为 return 的东西（但一般不会这么做）。

（3）箭头函数内的this

在没有箭头函数之前，内嵌函数由于本身是独立调用，若此时内嵌函数的 this 指向父函数，则一般是这么做：
...
有了箭头函数后就方便多了，箭头函数本身没有 this ，它的 this 实际上是父级作用域的 this（也可以说是定义该箭头函数的执行上下文所在的对象）：
...
const obj3 = {
  ...
  func() {
    return () => {
      console.log(this.a); // 指向 obj3，因为 return 的函数的父级作用域是 func 的局部作用域
    };
  },
  func2: () => {
    console.log(this); // 指向 window , 因为 func2 的父级作用于是全局作用域
  },
};
...
* 箭头函数不能作为构造函数自然也没有new规则。

（4）api 的回调函数

- api 的回调函数 this 指向由 api 作者决定

- 不管 api 内部如何实现，只要回调函数为箭头函数，就要用箭头函数 this 规则

- Vue 的 api 的回调函数的 this 指向 Vue 实例。

（5）实例

```
// 严格模式 window 会变 undefiend
const name = "window";
...
JS 中每个对象和函数都有自己的原型对象，可以调用，添加原型对象的属性和方法。

### 7.1 原型是什么

原型是一个对象，所以也叫原型对象。

只要是构造函数，都有显式原型属性指向它的原型，由于 js 中普通函数都是构造函数，所以普通函数也有原型，而箭头函数本身不能作为构造函数，所以箭头函数不睡构造函数也就没有原型。
...
只要是对象，包括字面量对象和构造函数 new 出的一个实例对象d，都有一个隐式原型属性 \_\_proto\_\_ 指向它的原型。
...
由于原型对象也是对象，所以也有隐式原型属性，且原型对象的构造者是顶级构造函数Object，所以函数的原型对象的原型就是Object的原型
...
原型链：原型与它的原型依次连接的链条，作用是查找属性和方法，调用属性方案时，先在自身上找，找不到再去原型上找，一直找到 Objject 的原型，找不到就返回 undefined 。
