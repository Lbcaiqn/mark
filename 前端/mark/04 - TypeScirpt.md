...
// 数组需要注意
const arr = [1, 'a'];  // 类型为 Array<numebr | string>
```
...
js 中对象的 key 只能是 string 或 Symbol，而 ts 对象属性的键的类型只能是number，string，symbol 或字面量，所以keyof any获得的联合类型是 number | string | symbol
...
* type 可以定义任何的类型，而 interface 只能定义对象类型

* type 不能重名，而 interface 可以，重名的会合并，相同属性会被覆盖

* typo 不能使用 extends ，而 interface 可以，但 type 也可以通过交叉类型 & 实现继承
