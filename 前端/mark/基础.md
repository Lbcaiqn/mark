...
优先级：默认 \< 外部/内部 \< 行内 。其中外部/内部优先级根据加载顺序

规范：

* 属性顺序：布局、盒子、背景、文本、字体 

* class 类名：BEM 规范（Block Element Modifier），分别是独立功能快、子元素、状态/外观修饰，作用是防止类名冲突以及更加语义化：
...
.box {
  ...
  /**盒子阴影，CSS3 新属性
   * 分别是水平偏移，垂直偏移，半径，颜色，内部，前两个必须有，默认阴影在外部，内部可设 inset
   * 可以用逗号隔开设置多个阴影 
   */
...
CSS 三角，原理是 border 连接处是斜线，当盒子内容宽高为 0 时，就会形成 4 个三角形，将其他 3 个设为透明即可得到一个三角形：

* 4 个三角，再两两 border 组合又得到 4 种，共 8 种三角形
...
基础选择器互相组合，结合优先级（从高到低）：

* \(\) 、:not\(\) 等

* 直接并列的如 \.a\.b\:hbover 、.a\:\:after 等 
...
/* 新的聚焦伪类选择器，只要自身或后代有聚焦 input 就选中 */
.box:focus-within {
  background-color: red;
}

/* 相对伪类选择器，选中符合特定条件的元素 */
/* 若 .label 第一个兄弟 input 有 data-required 属性，就选中 */
.label:has(+ input[data-required])::before {
  content: "*";
}

/* 首字母伪元素选择器，选中第一个字符，若首字符是标点符号，会继续选中直到非标点符号为止 */
p::first-letter {
  font-size: 50px;
}
...
/* 多行 */
.box {
  /* 注意不要设置 height ，因为这段代码是设置文本超出几行就显示省略号，若文本若还没有溢出盒子，就还会显示超出行的文本 */
  /* height: 100px; */
  /* padding 同理，会增加文本显示的区域 */
  // padding: 20px;

  /* 设置盒子模型，并垂直排列，设置多少行后开始隐藏，这 3 个属性有兼容性问题 */
  display: -webkit-box;
 ...
 * html：
   <div class="box">
     <div class="text-voerflow">...</div>
     <div class="text">文本</div>
   </div>
 * 利用浮动的文字环绕效果，增加一个浮动的省略号元素
 * 在 before 占位一个元素，将省略号压倒底部，再微调 before 高度和 text 的 margin-top
 * 最后 JS 控制省略号的显示隐藏，这里就不弄了
 */
.box {
  overflow: hidden;
  width: 200px;
...
- 确定声明值：没有冲突的属性直接是结果，html 标签的默认样式就是由默认样式表提供的

- 层叠冲突：
  
  - 比较重要性：作者样式表 \> 默认样式表
    
    * 这一步除 important 外和权重无关，因此通配符选择器才能清除浏览器默认样式
  
  - 比较特殊性：根据权重比较选择器的优先级
    
    * 注意作者样式表中，只有行内样式表有权重，而内部样式表和外部样式表则看加载顺序
  
  - 比较源次序：若还是有冲突，则后面的属性、选择器覆盖前面的

- 继承：若还有无 value 的属性，就继承，一般字体、文本属性可以继承，具体参考文档
  
  - line-height 继承：px 和比例直接继承，百分比会在父元素计算成 px 后再继承

- 使用默认值：若还有无 value 的属性，就使用默认值，注意默认值不是浏览器默认样式，而是文档规定的如 normal 、auto 等
  
  * 一些属性重要性不同，如 flex 会覆盖 width 的效果
  - 复合属性会覆盖前面的属性，如：
...
可用于任何属性，注意任何 CSS 操作都只是控制样式计算的前 2 部，也就是这些操作都不会到继承这一步；

* initial ：设为默认值，可用于清除浏览器默认样式，就不用文档查阅各种属性的默认值了，注意默认值不是浏览器默认样式，而是文档规定的默认值如 normal 、auto 等

* unset ：即不设置（默认样式表也是设置或的），效果是对于不可继承的属性使用 initial ，可继承则继承

* revert ：恢复为浏览器默认样式

* inherit ：应用父盒子对应属性的最终计算 value ，注意不是继承
...
<html lang="en">
  <head>
    <style>
      /**initial 、unset
       * 在第 2 步比较特殊性时由于和权重无关，所以 * 可以覆盖默认样式表中的标签选择器的样式
       * initial 设为初始值；unset 对于不可继承的属性使用 initial ，可继承则继承
       */
      * {
        /* margin: 0; */
        /* padding: 0; */
      ...
      * {
      ...
      /**inherit
        * 若不设置 a 的 color ，由于有默认样式也没有冲突，a 的 color 在第 1 步就确定了，不会继承
        * inherit 值为父盒子对应属性最终计算的值，这里即 .text 的 color ，.text 的 color 又继承于 .box
        */
      .box {
        color: red;
      }
...
由于这些都市通用属性，也可以用 all 设置全部属性，但是要慎用，如 all 设为 unset 会导致所有浏览器默认样式隐藏的如 \<html\> 、\<script\> 等显示出来，所有快盒也会变为行盒；

```
/* 这里只是简单处理，可能还会有其他被 unset 的默认样式 */
body,
body *:not(script) {
  all: unset;
  display: revert;
...
- 普通页面，header \+ 居中版心 \+ footer，header、footer与视口同宽

- 后台管理系统，整体和视口同宽，左导航栏，右内容区

移动端（调试可以手机和电脑连一个 wifi ，手机进入 电脑 ip:port ）；

- 响应式布局，限制屏幕缩放的最大与最小宽度

- 整体：顶部导航 \+ 内容区 \+ 底部 tabbar ，会区分 tabbar 页面和非 tabbar 页面
...
虽然会脱标，但不会压住一上来的文本和图片，因为浮动设置初中就是实现文字环绕效果。
...
<html lang="en">
  ...
  <style>
     ...
     * auto 的 width 为内容宽度
     * 包含块为最近的非 static 或设置了 transform 的盒子，所以一般父元素会设置 relative ，即子绝父相
     */
    .item2 {
      position: absolute;
      top: 50px;
     ...
     * 不保留位置，脱标，会转为 block 盒
     * auto 的 width 为内容宽度
     * 包含块为浏览器 viewport 或者最近的 transform 盒子
     */
    .item3 {
      position: fixed;
      top: 0;
    ...
    /**粘性定位
     * relative 基础上，在父元素内 fixed，等价于 relative + absolute ，所以包含快和绝对定位一样
     * 必须设置 top 才有效
     */
    .item4 {
      position: sticky;
...
* 层的上下文：z\-index 是基于包含块的，若包含块的祖先元素也设置了 z\-index ，可以看成两个 z-index 相加。
  ...
  <head>
    ...
    <style>
      ...
      .box1 {
        ...
        /*主轴方向上子盒子中间空隙的外边距，如主轴为 row ：
         * 不会有第一个子盒子的左外边距，不会有最后一个子盒子的右外边距
         * gap 的大小即中间空隙的距离，不像左右 margin 会叠加
        */
        gap: 20px;
      }
...
flex 配合 margin 可以更方便实现一些布局，margin 为 auto 就会自动占据剩余空间；
...
<html lang="en">
  <head>
    <style>
      ...
      /* 子元素 4 个方向的 margin 设为 auto 即可水平、垂直居中 */
      .box1 {
        width: 100px;
      }
      .box1 .item {
      ...
      /* 同理，auto 的 margin-left 会自动占据剩余空间，使得该子元素以及后面的子元素都右移，再设置 margin-right 为 auto 就能让中间某个元素居中 */
      .box2 .item:nth-child(3) {
        margin: 0 auto;
      }

      /* 列表的布局也可以用 margin 实现最后一行按顺序排列 */
      .box3 {
        flex-wrap: wrap;
        padding: 0;
        width: 212px;
  ...
  <head>
    ...
    <style>
      ...
      .box1 {
        ...
        /**行栅格
         * rid-template-rows 优先级高，剩余未指定的 grid-auto-rows 才生效 
         * 子元素可以不用设置 height ，会自动沾满整个栅格，而栅格的宽是内容宽度
         */
        /* grid-template-rows: repeat(3, 1fr); */
        /* grid-auto-rows: 10px 10px 10px; */

        /* 栅格水平、垂直排列方式，没有设置 grid-template-columns、grid-template-rows 才需要考虑 */
        /* justify-content: center; */
...
rem ，root em，CSS3 新增，与 em 不同的是只基于 的 font-size ，可配合多媒体查询实现进一步适配：
...
vw，vh ，viewport 宽度为 100vw ，vh同理，会随着 viewport 的变化自动适配，通常 vw 用的比较多。
...
vh 在调试工具中就是基于视口高度，但在真机中 vh 可能还会包含顶部地址栏和底部 tabbar ，导致 vh 偏大，以前的解决方法是用 JS 去掉多余高度，现在 CSS 新增了几个 vh 单位：

- svh ：最小 vh ，会去掉顶部地址栏和底部 tabbar ，真正基于视口高度

- lvh ：最大 vh ，初始和 svh 一样，若后续地址栏、tabbar 隐藏了，lvh 就会占用多出来的空间，即使后续地址栏和 tabbar 再次显示也不会恢复成 svh

- dvh ：动态计算的 vh ，完美替代 svh 和 lvh

vmin 、vmax ，分别是 vh 、vw 其中小/大的那个值：

```
/* 全屏显示图片为了不要滚动条，宽高为视口小的那个值，最终是一个正方形图片，所以原图宽高差距不要太多 */
.box > img {
  width: 100vmin;
  height: 100vmin;
}
...
```
/* 视口宽度在 320px 到 486px 之间时，以下样式适用 */
...
* 原因：移动端浏览器一般都有双击缩放或双击xxx的场景，为了监听出双击，在点击第一次后，若300ms内再次点击，就是双击，但是大部分情况下，300ms都是很难被用户接受的
...
* 块盒：BFC，即 块级格式化上下文（block formating context），让标准流的块盒形成独立的渲染区域，\<html\> 默认就是 BFC

* 行块盒：使用 IFC ，以及部分 BFC ，尽量不要使用行块盒进行布局，十分复杂，容易出错

#### 2.3.1 IFC

规则比较多，只介绍一个比较重要的：

若 html 标签中只有 ASCI 字符（空格、换行符等合法断点除外），是不会换行的，只有遇到合法断点或非 ASCI 字符才会换行，且非 ASCI 字符是新行的首位，若想要强制换行可以使用 <br> 或者 CSS 设置：
...
有 less 和 sass ，后缀名分别是 \.less ，\.scs ，安装；
...
解析成 .css ，单行注释和未使用的选择器、空内容的选择器不会保留：

```
npx lessc ./1.less ./1.css
npx sass ./1.scss ./1.css
```
...
* less 的 & 同一个简单选择器可以有多个，而 sass 的 & 只能在开头出现一次 
...
.box {
  ...
  // 同一个简单选择器 & 可以有多个但很少用，因为是 .box 整个复制过来，这里得到 .box-.box
  // 注意 sass 简单选择器的 & 只能在开头痴线一次，但复合选择器还是可以的如 &__text &__icon
  // &-& {
  //  color: #fff;
  // }
...
* less 用 @ 定义变量、而 sass 用 \$ ，sass 选择器变量需要加双引号

* 字符串：
  
  * less 、sass 保存属性值的变量加不加引号都行
  
  * less 保存选择器的变量不需要引号，而 sass 需要
  
  * less 使用字符串编译后会自动去掉引号，而 sass 需要手动去掉

* less 对象用花括号 ，访问用中括号；sass 用小括号，访问用内置函数
  
  * less 、sass 定义数组加不加小括号都行，访问都是用内置函数

* less 内置函数可以直接使用；旧版本 sass 也是，但较新版本的 sass 则需要导入才能使用
...
// 数组，下标从 1 开始，括号可以省略
@border-size: (1px, 2px, 3px);
.box {
  border: extract(@border-size, 1) solid #000;
}
...
// 较新版本的 sass 要求使用的函数必须从模块中导入；旧版本则直接使用全局函数
@use "sass:list";
@use "sass:map";
...
$border-size: (1px, 2px, 3px);
.box {
  border: list.nth($border-size, 1) solid #000;
}
...
变量运算，less 、sass 结果的单位都是最早出现的单位，区别：

* less 支持不同单位，而 sass：只能出现无单位和相同单位

* less 字符串拼接使用 \~"@\{\}" ，而 sass 使用 \#\{\} 且不需要引号
...
* less 变量没有作用域，都是全局的；而 sass 分为全局、局部作用域，若需要在局部作用域修改全局变量，需要加上 \!global ，否则会认为是在定义局部变量
...
* less 支持 arguments 、sass 不支持

* less 的 rest 参数必须用 rest 命名，而 sass 可以任意命名

* less 不支持代码块传入，sass 支持

```
// less

// 无参，无参时括号可以省略，但调用必须有括号
// .flex {}
// 无默认值
// .flex(@justify ,@align) {}
// 有默认值，
...
// @arguments 保存了所有参数；参数可以使用 rest 参数但是必须以 rest 命名
.border(@px, @rest...) {
  // border: @arguments;
  border: @px @rest;
}
...
// 有默认值，可以使用 rest 参数
@mixin flex($justify: center, $align: center, $border...) {
  display: flex;
  justify-content: $justify;
  align-items: $align;
  border: $border;
...
.box {
  // @include flex;
  @include flex(space-around, space-aroun, 1px solid, #000) {
    flex-direction: column;
  }
}
```
...
不像混合可以传参，但继承编译后会比混合提重更多代码，减少 CSS 体积。
...
.message--success:extend(.message) {
  font-size: 18px;
}

.message--error:extend(.message) {
  font-size: 20px;
}
```
...
sass 还支持用 % 定义抽象类，只用于继承，编译后不会生成抽象类：
...
less 没有函数，只能用混合，而 sass 有区别于混合的函数语法；
...
* less 使用 when ，sass 使用 if else ，sass 更语义化

* less 相等用 = 而 sass 用 == ；less 或逻辑使用逗号，而 sass 使用 or

* less 只能判断基本类型，而 sass 还可以判断数组、对象
...
// 使用 when 分支，使用 = < > <= >= 比较
// 下面可以理解为 if(...===...) elif(...===...)
.box-size(@type) when(@type = big) {
  width: 200px;
  height: 200px;
}
.box-size(@type) when(@type = small) {
  width: 100px;
  height: 100px;
}

// and 表示且，逗号表示或
// 下面可以理解为 if(... && ...) elif(... || ...) else
.box-theme(@x) when (@x > 10) and (@x < 20) {
  background-color: red;
}
.box-theme(@x) when (@x >= 20), (@x = 5) {
  background-color: green;
}
.box-theme(@x) when(default()) {
  background-color: blue;
}

// 类型判断，只能判断简单的类型
.text-style(@x) when (isnumber(@x)) {
  font-size: @x;
}
.text-style(@x) when (iscolor(@x)) {
  color: @x;
}
...
// 类型判断，数组判断为 "list" ，对象判断为 "map"
@mixin text-style($x) {
  @if (meta.type-of($x) == "number") {
    font-size: $x;
  } @else if (meta.type-of($x) == "color") {
    color: $x;
  }
}
...
* less 只能递归实现，且分支语法比较复杂；sass 则支持递归和 for 循环
...
// 循环生成属性值
.generate-margin(@i, @max, @base: 5px, @result: ~"") when (@i =< @max) {
  .generate-margin(@i + 1, @max, @base, ~"@{result} @{base}");
}
.generate-margin(@i, @max, @base, @result) when (@i > @max) {
  margin: @result;
}
.box {
  .generate-margin(1, 4, 5px);
}
```
...
//     @include get-items($i - 1);
//   }
// }
// @include get-items(3);

// 递归实现生成属性值
// @mixin get-margin($i, $px, $res: "") {
//   @if ($i > 0) {
//     @include get-margin($i - 1, $px, #{$res} #{$px});
//   } @else {
//     margin: $res;
//   }
// }
// .box {
//   @include get-margin(4, 5px);
// }
```
...
便利 list 、map ，注意 list 下标都是从 1 开始，区别：

* less 、sass 都可以便利 list  ，但 less 的 list 必须不能加小括号，否则会认为是一个值

* less 无法便利 map ；sass 则可以
...
虽然 less 、sass 用法一样，但是结合其他语法 sass 会更方便，以下是 sass 对多媒体查询的最佳实现；
...
// 混合
@mixin media($breakname) {
  $bp: map.get($breakpoints, $breakname);

  // 如果是数组，就有 min ，max 两个值
  @if (meta.type-of($bp) == "list") {
    $min: list.nth($bp, 1);
    $max: list.nth($bp, 2);

    @media (min-width: $min) and (max-witdh: $max) {
      @content;
    }
  }
  // 不是数组就只有一个 min 值
...
@media (min-width: 320px) and (max-witdh: 480px) {
  .box {
    background-color: red;
  }
}
@media (min-width: 769px) and (max-witdh: 1024px) {
  .box {
    background-color: green;
  }
}
...
```
// JS 调用 CSS 的多媒体查询，结果为 boolean
const theme = window.matchMedia("(prefers-color-scheme: dark)");
console.log(theme.matches);

// 监听系统切换主题，matches 会重新赋值
theme.addEventListener("change", () => console.log(theme.matches));
```
...
@function getVar($var-name) {
  $vars: map.get($theme, $current-theme);
...
@mixin useTheme() {
  @each $key, $value in $theme {
    ...
    // 默认即亮色模式
    @if ($current-theme == "line") {
      html & {
        @content;
      }
    } @else {
      html[data-theme="#{$current-theme}"] & {
        @content;
      }
    }
  }
...
less 、sass 基础用法一样，有几个缺点：

* 容易和原生 CSS 运行时的模块化混淆

* 不是完整的模块化，会全局污染，后导入的会覆盖前面导入的相同代码
...
也正是有很多缺点，新版本的 sass 编译后会提示已经不推荐使用以前的模块化了，新版本推荐使用 @use（less 还是使用老的模块化语法）：
...
// 默认命名空间是文件名，但若文件名是纯数字就需要手动重命名
// 若 as * 就可以直接使用公共成员但不推荐，多个文件 as * 也会报错因为会全局污染
@use "./2.scss" as a;
@use "./3.scss" as b;
...
// true ，使用类型转换函数可以省略包装类的调用，会自动加上
console.log(Number.parseInt === parseInt)
// 7 111 ，第二个参数为进制，默认为十进制 ，0 也看作十进制
console.log(parseInt("111", 2), parseInt("111", 0));
// NaN NaN ，进制有效范围为 2 ~ 36 ，不在这个范围，以及第一个参数不是该进制的合法值就得到 NaN
console.log(parseInt("111", 1), parseInt("3", 2));;
...
若需要更严格的比较，可以使用 ES6 的 Object\.is ，如 \+0 和 \-0 不同，NaN 和 NaN 相同：
...
- 由于设计初衷是为了消除二义性，所以箭头函数不能作为构造函数，不能 new、没有显示原型、没有自己的 this ，使用 this 其实就是在作用域链中找到的
...
注意，箭头函数只是没有显示原型，由于箭头函数也是对象，还是有隐式原型的：
...
```
// JSON ，缺点是 JSON API 自己的限制，优先数据类型无法拷贝
const func1 = (obj) => JSON.parse(JSON.stringify(obj));

// 扩展运算符，缺点是只能深拷贝一层
const func2 = (obj) => ({ ...obj });

// BOM ，缺点是兼容性差，需要高版本浏览器和高版本 node
const func3 = (obj) => structuredClone(obj);

// 也可以用一些有 postMessage 的 API 实现深拷贝，缺点是异步
function copy(obj) {
 ...
 * 构造函数只能 new ，不能调用
 * 非静态属性放在实例对象上
 * 非静态的 xxx() {} 的方法会放在原型上，且不可枚举，不能 new ；
   其他非静态方法放在实力对象上且可枚举，非箭头函数方法可以 new
 * 静态资源放在构造器对象自己身上，而不是原型上，静态方法可枚举，非箭头函数可 new ；
   形如 sstatic xxx() {} 的静态方法，也是放在构造器对象上而不是原型上，但不可枚举，不能 new
 * 静态代码块在代码初始运行就执行
...
// xxx() {} 的方法不可枚举，不能 new ；非静态放在原型上，静态放在构造器对象自身
[
  { target: B.prototype, funcNames: ["run0"], source: A.prototype },
  { target: B, funcNames: ["staticRun0"], source: A },
].forEach((item) => {
  for (const name of item.funcNames) {
    Object.defineProperty(item.target, name, {
      value: function () {
        /**条件说明
         * 只要 new ，typeof this 就是 "object"
         * 调用时，this 可能是：
           - B 的实力对象，第二、三个条件不满足
             obj.run0()
           - B.prototype ，第三个条件不满足
             B.prototype.run0()
           - B 自己（static 方法），typeof this 为 "function" ，三个条件都不满足
             B.staticRun0()
           - undefined（隐式丢失），三个条件都不满足
             const fn = obj.run0;
             fn();  // 从 B 原型上获取、从 B 获取静态方法也是一样)
           - 某个对象（new 的情况），此时构造函数不是 B ，属于不同的原型链，三个条件都满足
         */
        if (
          typeof this === "object" &&
          !(this instanceof B) &&
...
new o2.run2();
// 可枚举
// for (const key in o1) console.log(key);
// for (const key in o2) console.log(key);
// err ，因为是箭头函数
// new o1.run3();
// new o2.run3();

// 原型上的方法
console.log(A.prototype.run0);
console.log(B.prototype.run0);
// 不可枚举
for (const key in A.prototype) console.log(key);
for (const key in B.prototype) console.log(key);
// err ，不能 new
// new A.prototype.run0();
// new B.prototype.run0();

// 静态的属性方法，static xxx() {} 不可枚举
console.log({ ...A }, A.staticRun0);
console.log({ ...B }, B.staticRun0);
// ok
new A.staticRun1();
new B.staticRun1();
// err
...
// false true ，函数对象的构造者是 Function
console.log(func instanceof func, func instanceof Function);
// err ，右边必须是构造器，如构造函数、class
// console.log(obj instanceof func.prototype);
...
Object.defineProperty(obj, "a", {
  ...
  // 默认 false ，是否可枚举，如扩展运算符 ， for...in ，Object.keys() 等是否能获得这个属性
  enumerable: true,
...
/**修饰符，用于修改匹配模式 ---------------------------------------------------------
 * g：全局匹配，match 中会匹配所有可能的结果，exec 和 test 则借助 lastIndex 匹配所有结果
 * y：粘性模式，和 g 一样从 lastIndex 开始匹配，区别是 y 不会往后移动，而是只匹配一次
 * i：不区分大小写
 * m：多行模式，区分每一行的行首行尾，而不仅是整个文本的首尾
 * s：点任意匹配模式，让 . 能匹配包括换行符的任意字符
 * u：启用 unicode 模式，正确处理代理对字符
 */
// 默认情况区分大小写，只匹配第一个结果
var r1 = /a/;
 ...
 * test：
   ...
   - 不可访问分组
 * exec：
   - 找到第一个匹配结果，结果为 Array ，信息包含匹配结果、首字符下标等；找不到为 null
   - 可访问分组
 * match：
...
/**每个正则对象都有 lastIndex 属性 ---------------------------------------
 * lastIndex 保存上一次匹配成功的尾部 index + 1 ，下次再使用该正则对象时，就会从这个位置开始匹配
 * 匹配失败置为 0
 * 仅在 g 或 y 模式下有效，默认情况以及其他修饰符 lastIndex 始终为 0
 */
var regNum = /1/g;

// true 1 ，在 index 0 匹配成功，置为 1
console.log(regNum.test("123"), regNum.lastIndex);
// false 0 ，因为从 index 1 开始匹配，匹配失败置为 0
console.log(regNum.test("123"), regNum.lastIndex);
// true 1 ，重新从 index 0 开始匹配
console.log(regNum.test("123"), regNum.lastIndex);
// true 1 ，lastIndex 是可写的
regNum.lastIndex = 0;
console.log(regNum.test("123"), regNum.lastIndex);

// 借助 lastIndex ，就可以用 test 找到所有符合条件的 index
regNum.lastIndex = 0;
// 0 3 6
while (regNum.test("123123123")) console.log(regNum.lastIndex - 1);
...
// []  [] []，无法获取
console.log(Object.keys(m), Object.values(m), Object.entries(m));
// 正确做法
console.log([...m.keys()], [...m.values()], [...m.entries()]);
```
...
这些方法都是处理多个 Promise 的，接收一个可迭代对象，返回 Promise，若成员不是 Promise 则会转为 Promise ：
...
declare global {
  interface PromiseConstructor {
    myAll: (promiseList: Iterable<any>) => Promise<any>;
  }
}

Promise.myAll = <T>(promiseList: Iterable<any>) => {
  return new Promise((resolve, reject) => {
    const result: any[] = [];
...
若一个函数是异步的，那么其他函数内部调用这个函数时就都变为异步了，在函数式编程的环境中这是无法接受的，因为都变成了非纯函数：
...
解决方法，创建一个运行环境隔离异步边界，保留纯函数的风格：

```
const url = "https://jsonplaceholder.typicode.com/posts";

/**实现 ---------------------------------------------------------------------------------------------
 * 改造 fetch ，但为了不影响其他代码，只在这个运行环境中改造
 * 第一次调用 callback ，抛出异常，err 为 Promise ，保证同步
 * 等待异步结束，把结果保存到缓存中，再第二次调用 callback 得到缓存结果
 */

// 实际场景可能会多次请求，就需要保存多个，可以用请求 url 、请求头等综合作为 key ，这里为了方便就只处理只有一个请求的情况
interface ResCache {
  status: "fullfilled" | "rejected" | "pedding";
  value: any;
}

function use(callback: Function) {
  const oldFetch = window.fetch;
  const ResCache: ResCache = { status: "pedding", value: null };

  const newFetch = (...args: Parameters<typeof window.fetch>) => {
    if (ResCache.status === "fullfilled") return ResCache.value;
    else if (ResCache.status === "rejected") throw new Error("请求失败");

    const p = oldFetch(...args)
      .then((res) => res.json())
      .then((res) => {
        ResCache.status = "fullfilled";
        ResCache.value = res;
      })
      .catch((err) => {
        ResCache.status = "rejected";
        ResCache.value = err;
      });
...
const animationMap = new WeakMap<Element, any>();

// 若 el 在当前视口以及视口以上就不动画（为了防止滚动过快导致 top 判断不准确，这里视口高度小印点）
function isBeingViewport(el: Element) {
...
const ob = new IntersectionObserver((entries) => {
  for (const entry of entries) {
    ...
    if (entry.isIntersecting) {
      if (!isBeingViewport(entry.target)) break;
...
const vSlideIn: Directive = {
  mounted(el, dir) {
    if (!isBeingViewport(el)) return;

    const distance: number = dir.value || 100;
...
<template>
  <div class="box" v-for="i in 30">
    <div class="item" v-for="j in 7" v-slide-in="100"></div>
  </div>
</template>
...
- 对象属性可以使用 readonly 和 ? 修饰，而元组成员只能用 ? 修饰，但元组可以使用 rest 参数：
  
  * readonly 为只读，但只是限制属性自身，若属性是引用类型，则不限制成员的修改

- 数组、元组自身可以用 readonly 修饰，但不是表示数组/元组为只读，而是无法增删改成员：
  
  * 若成员是引用类型，则只限制引用类型自身修改，不限制更深层次的修改
  
  * 若数组/元组成员、对象属性是数组/元组，则它们的 readonly 是分开作用的`
...
// 2 ，元组可以获取 length 得到字面量类型，而数组的 length 为 number
type Len = ["a", "b"]["length"];

// 只读元组，无法增删改成员，但不是无法修改自身
let tup2: readonly [number, string, [boolean]] = [1, "a", [true]];
// ok ，自身可以重新赋值
tup2 = [1, "a", [true]];
// ok ，readonly 只作用成员自身
tup2[2][0] = false;
...
// 成员是 readonly 数组/元组
const tup3: readonly [1, readonly [2, 3]] = [1, [2, 3]];
// err ，tup3 readonly 的作用
// tup3[1] = [2, 3];
// err ，tup3[1] readonly 的作用
// tup[1][0] = 22;

// 可选
const tup4: [number, string?] = [1];

// rest
const tup5: [number, string?, ...any[]] = [1, "", true];
...
// ok ，数组成员是引用类型，可以修改他的属性；属性又是 readonly 的，则自身无法修改，但可以修改他的属性
arr[0].info.name = "qwer";
// err
// arr[0] = { info: { name: "qwer" }, child: [], items: [] };
// err
// arr[0].info = { name: "qwer" };
...
// ok ，readonly 属性的数组/元组，但数组/元组不是 readonly 的
arr[0].child[0] = 11;
// err
// arr[0].child = [];

// err ，readonly 的属性是数组/元组，且数组/元组也是 readonly 的
// arr[0].items[0] = 22;
// arr[0].items = [];
```
...
let num1 = 123;
const num2 = 123;
// 123 ，因此 let 未声明类型时，可以使用 as const
...
const data1 = [
...

  {
    id: "2",
    value: 2,
    children: [{ id: "22", value: 22 }],
    info: { name: "qwer" },
  },
];

/**as const，会尽可能变为 readonly 字面量
 * readonly [
 *   {
       readonly id: "1";
       readonly value: 1;
 ...
 *   {
       readonly id: "1"; 
       readonly value: 1;
       readonly info: { readonly name: "asdf" };

       // 注意这里和只使用 as const 不同，元组没有变为 readonly ，因为类型 Data 没定义
       readonly children: [{ 
         readonly id: "11";
         readonly value: 11;
         readonly info: { readonly name: "asdf" };
...
const data3 = [
  {
    ...
    value: 1,
    info: { name: "asdf" },
    children: [{ id: "11", value: 11, info: { name: "asdf" } }],
  },

  {
    ...
    value: 2,
    info: { name: "qwer" },
    children: [{ id: "22", value: 22, info: { name: "qwer" } }],
  },
] as const satisfies Data;
```

函数泛型的字面量类型 ，类似 as const，注意只有函数泛型才能使用 const ：
...
* 参考父类、子类即可：
  ...
  * readonly 属性和子类型无关；若属性是数组/元组，看数组/元组是否为 radonly 即可，和属性 readonly 无关
...
// true false true ，数组/元组的属性，readonly 是分开作用的
type A10 = { arr: any[] } extends { arr: readonly any[] } ? true : false;
type A11 = { arr: readonly any[] } extends { arr: any[] } ? true : false;
type A13 = { readonly arr: any[] } extends { arr: readonly any[] } ? true : false;
```
...
// true false false，先看数组/元组自身是否为 readonly ，再看成员
type A9 = readonly [[number]] extends readonly [readonly [number]]
  ? true
  : false;
type A10 = readonly [readonly [number]] extends readonly [[number]]
  ? true
  : false;
type A11 = readonly [readonly [number]] extends [readonly [number]]
  ? true
  : false;

// true true ，对象的类型关系和 readonly 属性无关
type A12 = readonly [{ a: number }] extends readonly [{ readonly a: number }]
  ? true
  : false;
type A13 = readonly [{ readonly a: number }] extends readonly [{ a: number }]
  ? true
  : false;

// true
type A14 = number[] extends (number | string)[] ? true : false;

// false true true ，元组和长度、顺序强相关
type A15 = [number] extends [number, number] ? true : false;
type A16 = [number] extends [number, number?] ? true : false;
type A17 = [number] extends [number, ...any[]] ? true : false;

// true ，元组是数组的子类型
type A18 = [number] extends number[] ? true : false;
```
