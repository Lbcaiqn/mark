...
* Vue3 的 \<template\> 不在必须有一个根元素，原理是 Vue3 没有根元素时会自动加上一个虚拟元素 \<Fragment\> ：
...
* React 的响应式系统：
  ...
  * React 为了更方便修改状态，提供了 useReducer

* React 的差值语法：
  
  * 使用单个大括号；数据绑定、事件绑定也是使用差值语法
  
  * 引用类型无法直接像 Vue 一样差值语法渲染为文本
...
* React 没有指令系统，也就没有 v\-show 、v\-model 等类似功能：
  
  * React 的 text 、textarea 等表单 onChange 对应原生的 input 事件；也没有对应的 change 事件
  
  * radio 、checkbox 等表单的 onChange 就对应原生的 change
...
// 自定义 hook
function useHidden(value: boolean = false): [boolean, () => void] {
...
function App() {
  ...
  /**状态，即响应式数据， -------------------------------------------------------------------
   * 通过 setCount 修改状态才是响应式的；禁止在组件顶层修改状态，否则会无线递归 render 函数
   * useStet 可以传入 value ，也可以传入函数
   * 创建状态不会修改原始数据
  ...
  // 表单 -------------------------------------------------------------------------------------
  const [inpValue, setInpValue] = useState("asdf");
  const [agree, setAgree] = useState(false);

  type ChangeEvent = React.ChangeEvent<HTMLInputElement>;

  const inpInput = (e: ChangeEvent) => setInpValue(e.target.value);

  // 条件渲染、列表渲染 ---------------------------------------------------------------------
  const [isShow, setIsShow] = useHidden(true);

  const data = [{ id: "0", text: "0" }];
  const [list, setList] = useState(data);
  const pushList = () => {
    const newId = Number(list.at(-1)!.id) + 1 + "";
    const newList = [...list, { id: newId, text: newId }];
    setList(newList);
  };
  ...
  return (
    <>
      ...
      {/* 表单 ---------------------------------------------------------------------------------- */}
      <p>
        <input type="text" value={inpValue} onChange={inpInput} />
        <span>{inpValue}</span>
      </p>
      ...
      {/* 条件渲染 */}
      <p>
        <button onClick={() => setIsShow()}>switch</button>

        {isShow && <span>类似 v-if</span>}
        {isShow ? <span>类似 v-if</span> : <></>}

        <span style={{ display: isShow ? "inline" : "none" }}>类似 v-show</span>
      </p>
...
* React 的 useMemo 用于缓存状态，状态变化就会重新执行，类似 Vue 的 computed 、watch

* React 的 useEffect 实现生命周期，也能实现类似 Vue 的 watch ；没有 nextTick
...
* useMemo 、useEffect ：
  ...
  * 第二个参数为依赖项：
    ...
    * 只有依赖的状态变化才会重新执行；若是非状态则没有效果
    
    * 默认为 undefined 则依赖所有状态，空数组则没有依赖项，就初始 mounted 执行一次
...
function App() {
  ...
  // useEffect ，一个 Hook 多个用途，实现类似 Vue 的 mounted 、updated/nextTick 、beforeUnmount 、watch
  useEffect(() => {
    // 类似于 Vue 的 mounted、updated/nextTick
    console.log("mounted");

    // 类似 beforeUnmounted ，但不完全相同，这个函数会再每次重新 render 时执行，而不一定是销毁组件；执行完成后再执行 useEffect
    // 也可以不 return
    return () => console.log("beforeUnmounted");
  }, [count]);
...
export interface CpnRef {
  cpn: HTMLDivElement | null;
  func: () => void;
}

/**获取子组件实例、资源
 * 需要 forwardRef + useImperativeHandle ，父组件才能获取
 * args：_ 即 props ，ref：父组件传入的 ref
 */
export const Cpn1 = forwardRef<CpnRef>((_, ref) => {
...
import { Cpn1, Cpn2 } from "./Cpn";
import type { CpnRef } from "./Cpn";

function App() {
  const divRef = useRef<HTMLDivElement | null>(null);

  const cpn1Ref = useRef<CpnRef | null>(null);
  const cpn2Ref = useRef(null);
...
function Home() {
  ...
  return (
    <>
      ...
      <p>
        <Link to="/about/aboutb"> to About b 2</Link>
      </p>
      <p>
...
方式二，React Router 6 早期写法，即方式三的简化版：
...
function App() {
  // 虽然是解构，但已经获取了整个 store ，其他不需要的 state 变化都会重新 render ，性能差一点
  const { count, increment, decrement, addBy, asyncIncrement } =
    useCounterStore();
...
const useCounterStore = create<CounterState>()(
  persist(
    ...
    // 默认 key 为 "zustend" ，使用 localStorage
    {
      name: "zustend",
