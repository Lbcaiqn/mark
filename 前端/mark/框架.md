...
* template ，编写 html ，最终编译为 render 函数：
  ...
  - 只能使用 v\-for 、v\-if 、v\-else\-if 、v\-else ，无法使用 v\-show
...
* 除了模板最外层和插槽外，其他地方也可以使用 template 作为包裹容器（不生成虚拟 DOM ，也不会渲染为真实 DOM），但 Vue2 、Vue3 编译行为不同：
 ...
 * 具有缓存功能，内部通过 dirty 判断是否需要重新执行 getter（dirty 初始为 true）：
   ...
   - 只有依赖的响应式数据修改了，dirty 设为 true，才会重新执行 getter ；而函数每次重新 render 都会执行
 * computed 一开始会执行一次（dirty 为 true），进行 track
 */
const a = ref(1);
const b = ref(2);
...
- 关于组件根节点：
  ...
  * 先要了解指令的作用范围：
    ...
    * 任何节点均可使用：v\-for 、v\-if 、v\-else\-if 、v\-else 、v\-pre 、v\-one （但是 pre 和 once 用在 slot 没有意义也没效果），如 template 、slot 这种不会生成虚拟 DOM 、不会渲染真实 DOM 的都能使用，因为这些指令只需要在Vue 控制生成就行，不需要绑定到真实 DOM 上
    
    * 只能用在元素节点、注释节点，不能用在文本节点以及 template 、slot 这种空节点：v\-show 、v\-cloak、自定义指令等，因为都需要绑定到真实的元素节点或注释节点上
    
    * 只能用在元素节点、组件，无法用在注释节点 ：v\-html 、v\-text 、v\-bind 、v\-on ，自重 v-bind 、v\-on 绑定组件时，是 props 和 emit 处理
    
    * 只能用在表单、组件：v\-model
  
  * keep\-alive 、Component 、Suspense 、Teleport 、router\-view 等最外层使用注释节点站位，所以可以用 v\-show 等；但当组件标签使用 v-show 等指令时，只会下发到元素节点，所以如果子组件跟标签是 keep-alive 等，或者多根节点，就会 err
  
  * Vue3 组件不一定需要根节点是因为会加上一层 Fragment ：
...
props 是一个 shallow readonly Proxy ，若修改 props 中的引用类型数据，即深度修改，就检测不到，常见的场景如传入表单数据，子组件 v\-model 绑定。可以参考 vueuse 的 useVModel 解决（简单实现，未实现原版的深度嵌套递归处理）：
...
Teleport 自身是和 template 一样是编译时，不会被渲染：
...
* 展示组件：只作展示，使用 props 提供的数据，emit 抛出事件给外部处理，最多只做一些简单逻辑如表单验证
  ...
  * 在 Vue2 中，可以设置配置项 functional 为 true 并使用 render 函数，或者 template 使用 functional 属性，使这个组件成为无状态无实例的函数式组件，正好可以用来做展示组件；无状态指无法定义 data ，只能使用 props ，无实例指由于没有状态就不需要创建组件实例，因此初始速度、内存消耗都会变小
...
interface UserFormData {
  dataName: Exclude<keyof UserData, "id">;
    ...
    <Teleport to="body">
      <div class="form-area" v-show="isShow">
        ...
        <form @submit="submit" @reset="$emit('reset')">
          <div v-for="(i, iIndex) in model">
            <input :type="i.type" v-model="i.value" />
            <div v-show="i.valid?.failed">不能为空</div>
          </div>
...
import type { UserData, UserFormData } from "./type";
...
const initialUserFormData: UserFormData[] = [
  {
    ...
    type: "text",
    value: "",
    valid: { failed: false, role: "" },
  },
];
const userFormData = ref(structuredClone(initialUserFormData));

const mode = ref<"add" | null>(null);

function clearUserFormData() {
  userFormData.value = structuredClone(initialUserFormData);
}
...
function addSubmit() {
  ...
  userList.push({
    ...Object.fromEntries(userFormData.value.map((i) => [i.dataName, i.value])),
    id: uuidv4(),
  } as UserData);
...
<template>
  ...
  <p>
    <span>test</span>
    <span>{{ userFormData[0].value }}</span>
  </p> 
  -->
...
* memory ：用于非浏览器环境（没有 BOM），如小程序、app 、桌面应用，这种模式会吧 url 存入到内存中的一个不可见对象中，再处理这个对象；
...
* React 的响应式系统：
  ...
  * React 由于 setXxx 是一个函数，而异步更新会吧这个函数放到异步队列，所以修改后无法同步得到最新 value ；而 Vue 是直接修改数据，可以同步得到最新 value ，只是异步更新 DOM
  
  * 重新执行 render 函数时：
    
    * useState 等 hook 返回的状态以及 useRef 、空依赖项的 useMemo 和 useCallback 会缓存（一般有返回值的 React Hook 都会缓存），而非状态变量会重新初始化，引用类型就变为新引用； Vue 的 setup 只执行一次
    
    * React 中若不想被初始化，最常用的就是使用 useRef 保存任意类型数据，非函数引用类型也可以用 useMemo ，函数可以用 useCallback
