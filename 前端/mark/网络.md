...
注意，html 的 \<meta\> 也可以模拟响应头，告诉浏览器一些处理行为，但最好还是服务器设置，因为 \<meta\> 可以被 JS 很轻易地删除，非常不安全。
...
* 模拟：
  ...
  <html lang="en">
    ...
    <script>
      ...
      /**测试
       * 如输入一个 script 脚本，但现代浏览器基本都过滤了这个行为，不会执行这个脚本
       * 如输入一个 img 标签，src 为不可访问的 url ，在 onerror 中注入脚本即可，这个行为浏览器不会阻止
       */
...
* 解决方法：
  ...
  * 现代浏览器有一套内容安全策略，后端可以设置 http 响应头，告诉浏览器哪些链接是可信的
