...
* 提高props的稳定性，稳定性值的是减少不必要的props，避免子组件过多更新

* 销毁组件前，必须要清空定时器、订阅等，防止内存溢出。因为组件销毁时只解绑指令，watch和事件等

* 不要过度分割，尤其是为了所谓的组件抽离将一些不需要渲染的组件特意抽出来，实际上组件的性能小号远大于DOM

* 代码分割，码分割是指构建工具将构建后的 JavaScript 包拆分为多个较小的，可以按需或并行加载的文件。通过适当的代码分割，页面加载时需要的功能可以立即下载，而额外的块只在需要时才加载，从而提高性能。
...
若图片未进入可视区域则不加载，当图片进入可视区域后才开始加载。
