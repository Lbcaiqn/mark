...
分为顶部和底部，2<=item数<=5，顶部只显示文本，底部显示icon和text
...
用于配置各页面特有的窗口样式，会覆盖全局的窗口配置，配置项与app.json一样
...
```
//下拉刷新可在app.json全局配置或各自页面的json文件配置，会重新渲染页面
//此外还可以自定义下拉行为，在data同级编写
onPullDownRefresh(){
  //自定义行为，默认是不重新渲染页面的，下拉也不会自动弹回
